...
const ToggleList = (id : string) => {
	lat_mapToArray(
		document.getElementsByClassName("msgList"),
		(k : any, obj : any) => obj
	).forEach(ele => {
	...
	lat_mapToArray(
		document.getElementsByClassName("btnToggle"),
		(k : any, obj : any) => obj
	).forEach(ele => {
...
const WrapMessageList = (props : {messages : TypeMessageExt[], cbLoadMessage : (message : TypeMessageExt) => void, isSent : boolean}) => {
	...
	return <WrapError fallback={msg}
	                  component={<LoadMessageList messages={props.messages}
	                                              cbLoadMessage={props.cbLoadMessage}
	                                              isSent={props.isSent}/>}/>;
};
...
const ListMessages = (props : {messages : TypeMessageList}) => {
	...
	return <>
		<div className={"la-container w3-padding-hor-12"}>
			<div className={"la-l2 la-s"}>
				<div className={"la-container la-l la-s"}>
			...
			<div className={"la-l8 la-s w3-border w3-padding-left"}>{mainComp}</div>
		</div>
	</>;
};
