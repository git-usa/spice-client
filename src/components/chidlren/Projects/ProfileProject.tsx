...
import {lat_cloneObject, lat_isValidArray} from "../../../modules/scripts/labject.js";

const showProfile = (tabId : string, project : TypeProfileProject, cbHandler : HandleComponentJax) => {
	const onShow = (c : typeof _lady, h : string, v : any, i : TypeProfileProject, carry : any) => {
		if(!carry) return;
		c._capitalWords();
...
const showTeams = (tabId : string, teams : TypeProfileTeam[], cbHandler : HandleComponentJax) => {
	...
	const mod = () => teams.map(team => {
		const t : any = lat_cloneObject(team);
		t.manager     = t.manager.name;
		t.managerId   = t.manager.id;
		return t;
	});
	...
	tab._showData(
		mod(),
		null,
		"name category manager status brief createdAt",
		(c : typeof _lady, h : string, v : any, i : TypeProfileTeam, carry : boolean) => {
			if(!carry) return;
			c._capitalWords();
			...
			switch(h){
				case "name":
					c._click(() => cbHandler("profile", {of : "team", by : i.id}));
					break;
				case "creator":
					c._click(() => cbHandler("profile", {of : "people", by : i.creatorId}));
					break;
		...
		}, null, true, {createdAt : {input : "date"}}, false, true
	)._classes("w3-table w3-table-all");
...
const ProfileProject = (props : {project : TypeProfileProject, cbHandler : HandleComponentJax}) => {
	...
	const project : any = lat_cloneObject(props.project);
